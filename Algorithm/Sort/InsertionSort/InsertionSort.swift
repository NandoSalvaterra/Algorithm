...
class InsertionSort {
    ...
    //Insertion Sort Algorithm O(N²) - Stable

    /*
     Ordenação por inserção (Insertion Sort), é um algoritmo de ordenação que, dado uma estrutura (array, lista) constrói uma sequência final com um elemento de cada vez, uma inserção por vez. Assim como algoritmos de ordenação quadrática, é bastante eficiente para problemas com pequenas entradas, sendo o mais eficiente entre os algoritmos desta ordem de classificação.
